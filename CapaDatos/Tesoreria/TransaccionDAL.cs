    ...
    public class TransaccionDAL:CadenaConexion
    {
        ...
        public List<TransaccionCLS> BuscarTransaccionesConsultaLimitada(int anioOperacion, int semanaOperacion, int codigoTipoOperacion, int codigoOperacion, int codigoCategoriaEntidad, string nombreEntidad, string fechaInicioStr, string fechaFinStr, string idUsuario, int esSuperAdmin)
        {
            ...
            using (SqlConnection conexion = new SqlConnection(cadenaTesoreria))
            {
                try
                {
                    ...
                    string filterNombreEntidad = string.Empty;
                    string filterRangoFechasOperacion = string.Empty;
                    string filterOperacionesConfiguradas = string.Empty;
                    ...
                    if (esSuperAdmin != 1)
                    {
                        filterOperacionesConfiguradas = "AND x.codigo_operacion in (SELECT codigo_operacion FROM db_tesoreria.config_usuario_operacion WHERE id_usuario = '" + idUsuario + "')";
                    }
        ...
        public List<TransaccionCLS> BuscarTransaccionesDepositosBancariosEnProceso(int anioOperacion, int semanaOperacion, int esSuperAdmin)
        {
            ...
            using (SqlConnection conexion = new SqlConnection(cadenaTesoreria))
            {
                try
                {
                    ...
                    INNER JOIN db_tesoreria.estado_solicitud_correccion h
                    ON x.codigo_estado_solicitud_correccion = h.codigo_estado_solicitud_correccion
                    WHERE x.codigo_estado = @CodigoEstadoRegistrado
                      AND x.anio_operacion = @AnioOperacion
                      AND x.semana_operacion = @SemanaOperacion  
                      AND x.complemento_Conta = 0
                      AND x.codigo_operacion =  @CodigoOperacionDepositoBancario
        ...
        public TransaccionComboCLS FillCombosConsultaTransaccionesLimitada(string idUsuario, int esSuperAdmin)
        {
            ...
            using (SqlConnection conexion = new SqlConnection(cadenaTesoreria))
            {
                ...
                {
                    conexion.Open();
                    using (SqlCommand cmd = new SqlCommand("db_tesoreria.uspFillComboFiltroConsultaTransaccionesLimitada", conexion))
                    {
                        // le indico que es un procedure
                        cmd.CommandType = CommandType.StoredProcedure;
