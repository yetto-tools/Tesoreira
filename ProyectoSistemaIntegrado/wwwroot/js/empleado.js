...
function fillCombosMotivosDeBaja() {
    fetchGet("Empleado/GetMotivosDeBaja", "json", function (rpta) {
        if (rpta == null || rpta == undefined || rpta.length == 0) {
            FillComboUnicaOpcion("uiCodigoMotivoBaja", "-1", "-- No existen motivos -- ");
        } else {
            FillCombo(rpta, "uiCodigoMotivoBaja", "codigoMotivoBaja", "nombre", "- seleccione -", "-1");
        }
    })
}


function fillCombosMotivosDeSuspension() {
    fetchGet("Empleado/GetMotivosDeSuspension", "json", function (rpta) {
        if (rpta == null || rpta == undefined || rpta.length == 0) {
            FillComboUnicaOpcion("uiCodigoMotivoSuspension", "-1", "-- No existen motivos -- ");
        } else {
            FillCombo(rpta, "uiCodigoMotivoSuspension", "codigoMotivoSuspension", "nombre", "- seleccione -", "-1");
        }
    })
}
...
function buscarEmpleados() {
    ...
    mostrarEmpleados(codigoEmpresa, codigoArea, codigoPuesto, codigoEstado, 0, 0);
}
...
function EditarEmpleado(obj) {
    Redireccionar("Empleado", "Edit/?codigoEmpleado=" + obj);
}
...
function EliminarEmpleado(obj) {
    ...
    $('#tabla tbody').on('click', 'tr', 'input:button', function () {
        ...
        fillCombosMotivosDeBaja();
        setI("uiTitlePopupDeBaja", "Dar de baja a Empleado");
        document.getElementById("ShowPopupDeBaja").click();
        ...
        set("uiFechaEgreso", "");
        set("uiFechaInicioSuspension", "");
        set("uiFechaFinSuspension","");
        set("uiObservaciones", "");
    });
}

function MostrarDataAdicionalMotivoDeBaja(obj) {
    let codigoMotivoBaja = parseInt(obj.value);
    let elementSuspension = document.getElementById('div-data-suspension');
    let elementOtrosMotivos = document.getElementById('div-data-egreso');
    let elementEgreso = document.getElementById("uiFechaEgreso");
    let elementFechaInicioSuspension = document.getElementById("uiFechaInicioSuspension");
    let elementFechaFinSuspension = document.getElementById("uiFechaFinSuspension");
    let elementMotivoSuspension = document.getElementById("uiCodigoMotivoSuspension");
    set("uiFechaInicioSuspension", "");
    set("uiFechaFinSuspension", "");
    set("uiObservaciones", "");
    set("uiFechaEgreso", "");
    if (codigoMotivoBaja == MOTIVO_DE_BAJA_SUSPENSION) {
        elementSuspension.style.display = 'block';
        elementOtrosMotivos.style.display = 'none';
        fillCombosMotivosDeSuspension();
        elementEgreso.classList.remove('obligatorio');
        elementFechaInicioSuspension.classList.add('obligatorio');
        elementFechaFinSuspension.classList.add('obligatorio');
        elementMotivoSuspension.classList.add('obligatorio');
    } else {
        elementSuspension.style.display = 'none';
        elementOtrosMotivos.style.display = 'block';
        FillComboUnicaOpcion("uiCodigoMotivoSuspension", "-1", "-- No Aplica -- ");
        elementEgreso.classList.add('obligatorio');
        elementFechaInicioSuspension.classList.remove('obligatorio');
        elementFechaFinSuspension.classList.remove('obligatorio');
        elementMotivoSuspension.classList.remove('obligatorio');
    }
}


function DarDeBajaEmpleado() {
    let errores = ValidarDatos("frmDeBajaEmpleado")
    if (errores != "") {
        MensajeError(errores);
        return;
    }

    var frmGuardar = document.getElementById("frmDeBajaEmpleado");
    var frm = new FormData(frmGuardar);
    Confirmacion(undefined, undefined, function (rpta) {
        fetchPost("Empleado/DarDeBajaEmpleado", "text", frm, function (data) {
            if (data == "OK") {
                Exito("Empleado", "Index", true);
            } else {
                MensajeError(data);
